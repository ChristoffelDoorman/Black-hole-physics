...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        cout << "Backreaction for cloud 2p with phi-component of shift only (fixed N, Psi and fields)" << endl;
    ...
    // only use third component of the shift
    List_comp used(1, 1);
    used.set(0)->set(0) = 3;
...
//    space.add_eq(syst, "eqB^i= 0", "bet^i", "dn(bet^i)"); // checked
    ...
    // Shift part written using the System_of_eqs functions (and not the space ones)
    // Inner BC (using some list of comp)
    syst.add_eq_bc (1, INNER_BC, "bet^i = 0", fixed) ;
    syst.add_eq_bc (1, INNER_BC, "bet^i = t / Psi^2 * nn^i + ome * m^i * r ", used) ;

    // Bulk equation and matching
    for (int d=1 ; d<ndom ; d++) {
        syst.add_eq_inside (d, "eqB^i=0") ;
        if (d!=ndom-1) {
            syst.add_eq_matching (d, OUTER_BC, "bet^i") ;
            syst.add_eq_matching (d, OUTER_BC, "dn(bet^i)") ;
        }
    }
...
//    }
    ...
    while (!endloop) {
        ...
        if (rank == 0) {
            ...
            char name[100];
            sprintf(name, "br_bet3_A%.4f_mu%.2f_lmbd%.4f.dat", A, mu, lambda);
